...
def select_stratified_samples(subset_datasets: list, max_samples: int):
    ...
    assigned proportionally (equal split by default) in the order the
    datasets are given. If a dataset is smaller than its quota, the shortfall
    is **not** backfilled from other subjects, so the total may be slightly
    below *max_samples*.

    Returns:
        A concatenated Hugging Face dataset containing the selected samples.
    """
...
def get_evaluation_dataset(max_samples: int = config.MAX_SAMPLES):
    ...
    selected proportionally (stratified) across subjects so that each subject
    contributes roughly ``max_samples / num_subjects`` items.

    Returns:
        A Hugging Face dataset with stratified samples from the configured
        MMMU subjects.
    """
    subset_datasets = []
